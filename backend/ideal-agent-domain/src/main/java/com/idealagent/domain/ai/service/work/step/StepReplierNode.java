...
public class StepReplierNode extends StepNodeSupport {
    public StepReplierNode(IChatClientArmory armory, IMcpToolService mcpToolService, WorkChatGateway chatGateway, WorkJsonParser parser, RuntimeMessageBuilder messageBuilder) {
    ...
    public void execute(ExecuteRequestEntity request, ExecuteContext context, WorkEventSink sink) {
        try {
            ...
            JsonNode result = parser.parseObject(call(flow, flow.getUserPrompt().formatted(request.getUserMessage(), history), request));
